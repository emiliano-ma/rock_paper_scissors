describe('Game logic works when computer choices Paper', () => {
  beforeEach(() => {
    cy.visit('/', {
      onBeforeLoad: (selectionHandler) => {
        cy.stub(selectionHandler.Math, 'floor').returns(1);
      }
    })
  ...
  context ('player choices Rock', () => {
    ...
    it('displays correct computer choice', () => {
      cy.get('#computer-choice').should('contain', 'choise is: Paper')
    })
